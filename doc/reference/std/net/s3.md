...
Credentials are resolved using the standard AWS credential chain:

1. Explicit arguments (`access-key:`, `secret-key:`, `region:`)
2. Environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_DEFAULT_REGION`)
3. AWS credentials file (`~/.aws/credentials`)
4. AWS config file (`~/.aws/config` for region)

The `profile:` argument selects which profile to use from the credentials/config files.
If not specified, it defaults to the `AWS_PROFILE` environment variable, or `"default"`.

If `access-key` or `secret-key` cannot be resolved through any of these methods,
an `S3ClientError` is raised.
