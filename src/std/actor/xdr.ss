...
(def (xdr-float-read port)
  (let* ((bytes (make-u8vector 8))
         (ilen (buffer-read-subu8vector bytes 0 8 port)))
    (if (fx= ilen 8)
...
(def (xdr-binary-read port K)
  (let* ((len (xdr-read-object port))
         ...
         (ilen (buffer-read-subu8vector buf 0 len port)))
    (if (fx= len ilen)
      (K buf)
      (raise-xdr-error 'xdr-read "premature port end" port))))
  ...
  (cond
   ...
   (else
    (let* ((sign (if (negative? obj) 1 0))
      ...
      (let lp ((k 0) (value value))
        (when (fx< k bytes)
          (buffer-write-u8 (bitwise-and value #xff) port)
...
(def (xdr-values-write obj port)
  (buffer-write-u8 xdr-proto-type-values port)
  (xdr-vector-like-write obj 0 port))

(def (xdr-vector-write obj port)
  (buffer-write-u8 xdr-proto-type-vector port)
  (xdr-vector-like-write obj 0 port))

(def (xdr-u8vector-write obj port)
  (buffer-write-u8 xdr-proto-type-u8vector port)
...
(def (xdr-hash-write obj port)
  (buffer-write-u8 xdr-proto-type-hash port)
  (let (testf (##vector-ref obj 2))     ; _system#.scm
    (cond
     ((or (not testf)
          (eq? testf eq?)
          (eq? testf ##eq?))
      (buffer-write-u8 xdr-proto-type-hash-eq port))
     ((or (eq? testf eqv?)
          (eq? testf ##eqv?))
      (buffer-write-u8 xdr-proto-type-hash-eqv port))
     (else
      (buffer-write-u8 xdr-proto-type-hash-equal port))))
  (xdr-inline-list-write (hash->list obj) port))

(def (xdr-opaque-write obj port)
  (buffer-write-u8 xdr-proto-type-opaque port)
...
(def (xdr-structure-write obj port)
  (let (type-id (##type-id (object-type obj)))
    (cond
     ((xdr-type-registry-get type-id)
      => (lambda (xdr)
           (buffer-write-u8 xdr-proto-type-structure port)
