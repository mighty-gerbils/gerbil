...
(defrule (test-signature-vector type priv pub msg sig)
...
(defrule (test-signature-type type (priv pub msg sig) ...)
  (begin (test-signature-vector type priv pub msg sig) ...))
(defrule (test-signatures (type vectors ...) ...)
  (begin (test-signature-type type vectors ...) ...))

(def pkey-test
  (test-suite "test :std/crypto/pkey"
    (test-case "test signature vectors"
      (when EVP_PKEY_ED25519 ; This functionality requires OpenSSL 1.1.1
        (test-signature-type
         EVP_PKEY_ED25519
         ;; test vectors from RFC 8032 https://tools.ietf.org/html/rfc8032
