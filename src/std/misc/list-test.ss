...
(def list-test
  (test-suite "test :std/misc/list"
    ...
    (test-case "test partition-by"
      (check (partition-by [])                => [])
      (check (partition-by [1 2] key: #!void) => [])
      (check (partition-by [1 2])             => [[1] [2]])
      (check (partition-by [1 2 3 1])         => [[1 1] [2] [3]])
      (check (partition-by [1 2 3 1] test: 2) => [[2] [1 3 1]])
      (check (partition-by '((1 a) (2 b) (1 b)) key: car)
             => '(((1 a) (1 b)) ((2 b))))
      (check (partition-by '((1 a) (2 b) (1 b) (3 a)) key: cadr test: 'a)
             => '(((1 a) (3 a)) ((2 b) (1 b))))
      (check (partition-by ["a" "b" "A"] test: string-ci=?)
             => [["a" "A"] ["b"]])
      (check (partition-by ["a" "b" "A"] test: (lambda (a b) (andmap number? [a b])))
             => [["a" "b" "A"]]))
