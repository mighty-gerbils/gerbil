...
;; vyzo: this is not thread-safe, use a parameter with identifier-rules
...
;;       so we can make the refs unsafe, barring bugs in dorai's code...
(define-syntax string-ref
  (syntax-rules ()
    ((_ s x) (##string-ref s x))))
...
(defrule (pregexp-error where irritants ...)
  (BUG where "pregexp internal error" irritants ...))
...
;; vyzo: opt-lambda and range checking so that fixnum arithmetic and unchecked
;;       string-refs  can be used
(define pregexp-match-positions
