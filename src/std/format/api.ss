...
(begin-syntax
  ...
  (def (fold-format-ops-for-stx stx writer env ops)
    (with-syntax ((writer writer) (env env))
      (with-identifiers ((writer.format-char    #'writer #'writer ".format-char")
        ...
        (match rest
          ([op . rest]
           (match op
             (['char arg]
              (with-syntax ((arg arg))
                (cons #'(writer.format-char arg env)
                      result)))
             (['int arg how flags width]
              (with-syntax ((arg arg) (how how) (flags flags) (width width))
             ...
             (['float arg how flags width precision result]
              (with-syntax ((arg arg) (how how) (flags flags) (width width) (precision precision))
                (cons #'(let (env (@format-env env
                                               (flags: flags)
                                               (width: width)
                                               (precision: precision)
...
(defsyntax-case fprintf ()
  ((_ output-target fmt-string arg ...)
   ...
   (let* ((ops    (fold-format-string-for-stx stx (stx-e #'fmt-string) #'(arg ...)))
          ...
          (env    (syntax-local-temp '$env))
          (ops    (fold-format-ops-for-stx stx writer env ops)))
     (with-syntax ((writer writer) ((op ...) ops))
