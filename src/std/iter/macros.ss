  ...
  (syntax-case stx ()
    ((_ bind body)
     ...
     (syntax-case #'bind ()
       ((hd iter-expr . traits)
        (if (identifier? #'hd)
          (with-syntax ((dispatch (make-lambda #'hd #'traits #'body)))
            #'(for1-dispatch dispatch iter-expr))
          (with-syntax ((dispatch (make-match-lambda #'hd #'traits #'body)))
            #'(for1-dispatch dispatch iter-expr))))))))
...
(defsyntax-case for* ()
  ((_ (bind) body rest ...)
   (for-binding? #'bind)
   #'(for1 bind (let () body rest ...)))
  ((_ (bind bind-rest ...) body rest ...)
   (for-binding? #'bind)
   (if (null? #'(bind-rest ...))
     #'(for1 bind (let () body rest ...))
     #'(for1 bind (for* (bind-rest ...) body rest ...)))))
