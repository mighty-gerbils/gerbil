  ...
  (let (slen (##string-length str))
    (let lp ((i 0) (j 0))
      (when (##fx< i slen)
        (let* ((char (##string-ref str i))
               (c (##char->integer char)))
          (if (and (fixnum? c) (##fx<= c #x7f))
            (begin
              (##u8vector-set! u8v j c)
              (lp (##fx+ i 1)
                  (##fx+ j 1)))
            (utf8-encode-multibyte! str u8v i j)))))))

(def (utf8-encode-multibyte! str u8v i j)
  (def (encode1 c j bytes bits)
    (let (u8 (##fx+ (##fx- #xff (##fxarithmetic-shift-right #xff bytes))
                    (##fxarithmetic-shift-right c bits)))
      (##u8vector-set! u8v j u8)))

  (def (encode+ c j bytes)
    (let lp ((c c) (j j) (bytes bytes))
      (when (##fx> bytes 0)
        (let (u8 (##fx+ #x80 (##fxand c #x3f)))
          (##u8vector-set! u8v j u8)
          (lp (##fxarithmetic-shift-right c 6)
              (##fx- j 1)
              (##fx- bytes 1))))))
  ...
  (def (encode5 c j)
    ...
    (encode+ c (##fx+ j 4) 4))
  (def (encode6 c j)
    (let (u8 (##fx+ #xfc (arithmetic-shift c -30)))
      (##u8vector-set! u8v j u8)
      (let ((j+1 (##fx+ j 1))
            (u8 (##fx+ #x80 (##fxand (arithmetic-shift c -24) #x3f))))
        (##u8vector-set! u8v j+1 u8)
        (encode+ (bitwise-and c #xffffff) (##fx+ j+1 4) 4))))
        ...
        (let (byte (##u8vector-ref u8v i))
          (if (##fx<= byte #x7f)
            (begin
              ...
              (lp (##fx+ i 1)
                  (##fx+ j 1)))
            (utf8-decode-multibyte! u8v str i j)))
        j))))

(def (utf8-decode-multibyte! u8v str i j)
  (def u8vlen (##u8vector-length u8v))
