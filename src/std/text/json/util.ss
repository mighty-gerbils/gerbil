...
(def (trivial-json-object->struct strukt json (defaults #f))
  ...
  (for (((values k v) (in-hash json)))
    (let (p (get-pos k))
      ...
      (##structure-set! object p v strukt 'trivial-json-object->struct)))
  (def unbounds
    (with-list-builder (c)
     (for ((p (in-range 1 n)))
       (let ((b? (vector-ref bound? p))
         ...
         (cond
          (b? (void))
          ((hash-key? defaults slot) (##structure-set! object p (hash-ref defaults slot) strukt 'trivial-json-object->struct))
          (else (c slot)))))))
