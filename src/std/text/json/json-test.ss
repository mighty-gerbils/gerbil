...
(def (check-decode obj str)
  (check (call-with-input-string str read-json) => obj))
(def (check-encode-decode= obj (test equal?))
  (parameterize ((json-sort-keys #f))
    (checkf test (call-with-input-string (call-with-output-string (cut write-json obj <>)) read-json)
            obj))
  (let (p (open-output-u8vector))
    (write-json obj p)
    (let (q (open-input-u8vector (get-output-u8vector p)))
      (checkf test (read-json q) obj))))
(def (check-encode-decode obj str (test equal?))
  (check-encode obj str)
  (check-decode obj str)
  (check-encode-decode= obj test))
...
(def json-test
  (test-suite "test :std/text/json"
    (test-case "test object encoding and decoding"
      ...
      (check-encode (foo 23 41) "{\"a\":23,\"b\":41}")
