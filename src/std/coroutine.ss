...
(defstruct cort ((k :- :procedure)
                 end?
                 res)
  final: #t)

(defstruct cothr ((mx :- :mutex)
                  (cv :- :condvar)
                  (state :- :symbol)
                  val)
  final: #t)

(def (coroutine (proc : :procedure) . args)
  (let/cc k
    (coroutine-start! k (if (null? args) proc (cut apply proc args)))))

(def (cothread (proc : :procedure) . args)
  => :thread
...
(def (coroutine-start! (k :- :procedure) (thunk :- :procedure))
...
(def (coroutine-continue! (c :- cort) args)
  (if c.end?
    (apply values c.res)
    (let/cc kk
      (let (k c.k)
...
(def (coroutine-yield! (c :- cort) args)
  (let/cc kk
    (let (k c.k)
      (set! c.k kk)
      (apply k args))))
...
(def (cothread-start! (c :- cothr) (proc :- :procedure) args)
...
(def (cothread-continue! (c :- cothr) val)
  (mutex-lock! c.mx)
  (case c.state
    ((run)
     (set! c.val val)
...
(def (cothread-yield-values! (c :- cothr) val)
  (mutex-lock! c.mx)
  (case c.state
    ((continue)
     (let (kont c.val)
...
(def (cothread-signal! (c :- cothr) state val)
  (mutex-lock! c.mx)
