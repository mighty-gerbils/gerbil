    ...
    ((_ t)
     (with-syntax ((make-@vector              (format-id #'t "make-~avector" #'t))
       ...
       #'(begin
           ...
           (defvector-concatenate @vector-concatenate @vector? make-@vector @vector-length @vector-copy!)
           ...
           (defvector-fold @vector-fold @vector? @vector-length @vector-ref)
           (defvector-fold-right @vector-fold-right @vector? @vector-length @vector-ref)
           (defvector-map @vector-map @vector? make-@vector @vector-length @vector-ref @vector-set!)
           (defvector-map! @vector-map! @vector? @vector-length @vector-ref @vector-set!)
           (defvector-for-each @vector-for-each @vector? @vector-length @vector-ref)
           (defvector-count @vector-count @vector? @vector-length @vector-ref)
           (defvector-cumulate @vector-cumulate @vector? make-@vector @vector-length @vector-ref @vector-set!)
           ...
           (defvector-index @vector-index @vector? @vector-length @vector-ref)
           (defvector-index-right @vector-index-right @vector? @vector-length @vector-ref)
           (defvector-skip @vector-skip @vector-index)
           (defvector-skip-right @vector-skip-right @vector-index-right)
           (defvector-any @vector-any @vector? @vector-length @vector-ref)
           (defvector-every @vector-every @vector? @vector-length @vector-ref)
           (defvector-partition @vector-partition @vector? make-@vector @vector-length @vector-ref @vector-set! @vector-count)
           (defvector-filter @vector-filter @vector? make-@vector @vector-length @vector-ref @vector-set! @vector-count)
           (defvector-remove @vector-remove @vector-filter)
           (defvector-swap! @vector-swap! @vector? @vector-length @vector-ref @vector-set!)
...
(defrules defvector-unfold ()
  ((_ @vector-unfold make-@vector @? @vector-set!)
   (def/c (@vector-unfold f len seed)
     (@contract (procedure? f))
     (declare (not safe))
...
(defrules defvector-unfold-right ()
  ((_ @vector-unfold-right make-@vector @? @vector-set!)
   (def/c (@vector-unfold-right f len seed)
     (@contract (procedure? f))
     (declare (not safe))
     (let (v (make-@vector len))
...
(defrules defvector-copy! ()
  ((_ @vector-copy! @vector? @vector-length @vector-ref @vector-set!)
   (def/c (@vector-copy! to at from start end)
     (@contract (@vector? to)
                (@vector? from)
                (and (fixnum? at) (fx<= 0 at (fx1- (@vector-length at))))
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length from))))
                (and (fixnum? end) (fx<= start end (@vector-length from)))
                (fx<= (fx- end start) (fx- (@vector-length to) at)))
     (declare (not safe))
     (let lp ((i start) (j at))
       (when (fx< i end)
...
(defrules defvector-reverse-copy! ()
  ((_ @vector-reverse-copy! @vector? @vector-length @vector-ref @vector-set!)
   (def/c (@vector-reverse-copy! to at from start end)
     (@contract (@vector? to)
                (@vector? from)
                (and (fixnum? at) (fx<= 0 at (fx1- (@vector-length at))))
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length from))))
...
(defrules defvector-concatenate ()
  ((_ @vector-concatenate @vector? make-@vector @vector-length @vector-copy!)
   (def/c (@vector-concatenate lst)
     (@contract (andmap @vector? lst))
     (declare (not safe))
...
(defrules defvector-fold ()
  ((_ @vector-fold @vector? @vector-length @vector-ref)
   (def*/c @vector-fold
     ((kons knil v)
      (@contract (procedure? kons)
                 (@vector? v))
      (declare (not safe))
     ...
     ((kons knil v . vs)
      ...
      (declare (not safe))
      (let* ((vs (cons v vs))
             (end (apply fxmin (map (cut @vector-length <>) vs))))
        (let lp ((i 0) (r knil))
          (if (fx< i end)
            (lp (fx1+ i)
...
(defrules defvector-fold-right ()
  ((_ @vector-fold-right @vector? @vector-length @vector-ref)
   (def*/c @vector-fold-right
     ((kons knil v)
      (@contract (procedure? kons)
                 (@vector? v))
      (declare (not safe))
     ...
     ((kons knil v . vs)
      ...
      (declare (not safe))
      (let* ((vs (cons v vs))
             (last (fx1- (apply min (map (cut @vector-length <>) vs)))))
        (let lp ((i last) (r knil))
          (if (fx>= i 0)
            (lp (fx1- i)
                (apply kons r (map (cut @vector-ref <> i) vs)))
...
(defrules defvector-map ()
  ((_ @vector-map @vector? make-@vector @vector-length @vector-ref @vector-set!)
   (def*/c @vector-map
     ((f v)
      (@contract (procedure? f)
                 (@vector? v))
     ...
     ((f v . vs)
      ...
      (declare (not safe))
      (let* ((vs (cons v vs))
             (len (apply fxmin (map (cut @vector-length <>) vs)))
             (u (make-@vector len)))
        (let lp ((i 0))
          (if (fx< i len)
            (begin
...
(defrules defvector-map! ()
  ((_ @vector-map! @vector? @vector-length @vector-ref @vector-set!)
   (def*/c @vector-map!
     ((f v)
      (@contract (procedure? f)
                 (@vector? v))
...
(defrules defvector-for-each ()
  ((_ @vector-for-each @vector? @vector-length @vector-ref)
   (def*/c @vector-for-each
     ((f v)
      (@contract (procedure? f)
                 (@vector? v))
      (declare (not safe))
...
(defrules defvector-count ()
  ((_ @vector-count @vector? @vector-length @vector-ref)
   (def*/c @vector-count
     ((pred v)
      (@contract (procedure? pred)
                 (@vector? v))
      (declare (not safe))
      (let (end (@vector-length v))
        (let lp ((i 0) (r 0))
...
(defrules defvector-cumulate ()
  ((_ @vector-cumulate @vector? make-@vector @vector-length @vector-ref @vector-set!)
   (def/c (@vector-cumulate f knil v)
     (@contract (procedure? f)
                (@vector? v))
     (declare (not safe))
     (let* ((len (@vector-length v))
...
(defrules defvector-index ()
  ((_ @vector-index @vector? @vector-length @vector-ref)
   (def*/c @vector-index
     ((pred v)
      (@contract (procedure? pred)
                 (@vector? v))
...
(defrules defvector-index-right ()
  ((_ @vector-index-right @vector? @vector-length @vector-ref)
   (def*/c @vector-index-right
     ((pred v)
      (@contract (procedure? pred)
                 (@vector? v))
      (declare (not safe))
     ...
     ((pred v . vs)
      ...
      (declare (not safe))
      (let* ((vs (cons v vs))
             (start (fx1- (apply fxmin (map (cut @vector-length <>) vs)))))
        (let lp ((i start))
          (if (fx>= i 0)
            (if (apply pred (map (cut @vector-ref <> i) vs))
              i
...
(defrules defvector-any ()
  ((_ @vector-any @vector? @vector-length @vector-ref)
   (def*/c @vector-any
     ((pred v)
      (@contract (procedure? pred)
                 (@vector? v))
...
(defrules defvector-every ()
  ((_ @vector-every @vector? @vector-length @vector-ref)
   (def*/c @vector-every
     ((pred v)
      (@contract (procedure? pred)
                 (@vector? v))
...
(defrules defvector-partition ()
  ((_ @vector-partition @vector? make-@vector @vector-length @vector-ref @vector-set! @vector-count)
   (def/c (@vector-partition pred v)
     (@contract (procedure? pred)
                (@vector? v))
     (declare (not safe))
...
(defrules defvector-filter ()
  ((_ @vector-filter @vector? make-@vector @vector-length @vector-ref @vector-set! @vector-count)
   (def/c (@vector-filter pred v)
     (@contract (procedure? pred)
                (@vector? v))
     (declare (not safe))
     (let* ((count (@vector-count pred v))
...
(defrules defvector-swap! ()
  ((_ @vector-swap! @vector? @vector-length @vector-ref @vector-set!)
   (def/c (@vector-swap! v i j)
     (@contract (@vector? v)
                (and (fixnum? i) (fx<= 0 i (fx1- (@vector-length v))))
                (and (fixnum? j) (fx<= 0 j (fx1- (@vector-length v)))))
     (declare (not safe))
...
(defrules defvector-fill! ()
  ((_ @vector-fill! @vector-length @vector-set! @? @vector?)
   (def/c (@vector-fill! v fill (start 0) (end (@vector-length v)))
     (@contract (@vector? v)
                (@? fill)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
     (declare (not safe))
     (let lp ((i start))
       (when (fx< i end)
         (@vector-set! v i fill)
...
(defrules defvector-reverse! ()
  ((_ @vector-reverse! @vector-length @vector-ref @vector-set! @vector?)
   (def/c (@vector-reverse! v (start 0) (end (@vector-length v)))
     (@contract (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
     (declare (not safe))
...
(defrules defvector-unfold! ()
  ((_ @vector-unfold! @vector-length @vector-set! @vector?)
   (def/c (@vector-unfold! f v start end seed)
     (@contract (procedure? f)
                (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
...
(defrules defvector-unfold-right! ()
  ((_ @vector-unfold-right! @vector-length @vector-set! @vector?)
   (def/c (@vector-unfold-right! f v start end seed)
     (@contract (procedure? f)
                (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
...
(defrules defvector-reverse-vector->list ()
  ((_ reverse-@vector->list @vector-length @vector-ref @vector?)
   (def/c (reverse-@vector->list v (start 0) (end (@vector-length v)))
     (@contract (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
     (declare (not safe))
...
(defrules defvector->vector ()
  ((_ @vector->vector @vector-length @vector-ref @vector?)
   (def/c (@vector->vector v (start 0) (end (@vector-length v)))
     (@contract (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
     (declare (not safe))
...
(defrules defvector-generator ()
  ((_ make-@vector-generator @vector-length @vector-ref @vector?)
   (def/c (make-@vector-generator v (start 0) (end (@vector-length v)))
     (@contract (@vector? v)
                (and (fixnum? start) (fx<= 0 start (fx1- (@vector-length v))))
                (and (fixnum? end) (fx<= start end (@vector-length v))))
     (declare (not safe))
