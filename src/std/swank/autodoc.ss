		  ...
		  "" (lambda ()
		       (map write
			    (if (pair? x)
			      (cdr x) (list x)))))
		(lp (cdr kons)))))))
(def (swank-pp-procedure sym (noargs '/args/))
  (def proc (procedure-search sym))
  (if (or (null? proc) (not proc))
    `(,sym . ,noargs)
      (cons sym
	    (map string->symbol (foldl (lambda (t p) (append (single-dots (cdr t)) p)) []
	   (car (assget "args" (cdr proc))))))))
