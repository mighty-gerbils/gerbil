...
(import
  (for-syntax (only-in :std/sugar defrule syntax-eval))
  (only-in :gerbil/core/expander syntax->datum core-expand1)
  (only-in :std/sugar defrule syntax-eval)
...
(def quasiquote-test
  (test-suite "test gerbil quasiquote"
    (gx#current-expander-context (gx#import-module "quasiquote-test.ss" #f #t))
    (test-case "foo"
