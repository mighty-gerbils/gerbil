...
;; Temporary, for backwards compatibility only, a wrapper around the latter (to be renamed)
(def (c4-linearize rhead supers
                   get-precedence-list: get-precedence-list
                   struct: (struct? (lambda (_) #f))
                   eq: (eq eq?)
  ...
  (cond
   ...
   (else ;; 2 parents or more: it's effectively multiple inheritance
    (let (;; reverse list or reverse (precedence lists or local order lists) to merge
      ...
      (defvalues (make-tab tab-ref tab-set! tab-remove! tab->list)
        (cond
         ;; Use Gambit hash-tables as an optimization when possible
         ((eq? eq eq?)
          (values (lambda () (make-table test: eq?)) table-ref table-set! table-set! table->list))
         ((eq? eq eqv?)
          (values (lambda () (make-table test: eqv?)) table-ref table-set! table-set! table->list))
         ((eq? eq equal?)
          (values (lambda () (make-table test: equal?)) table-ref table-set! table-set! table->list))
         (else
          (values (lambda ()
                    (def this (cons #f []))
                    (set-car! this this)
                    this)
                  (lambda (tab key default)
                    (cond ((assoc key (cdr tab) eq) => cdr)
                          (else default)))
                  (lambda (tab key value)
                    (cond ((assoc key (cdr tab) eq) =>
                           (lambda (entry) (set-cdr! entry value)))
                          (else
                           (let ((tail [[key :: value]]))
                             (set-cdr! (car tab) tail)
                             (set-car! tab tail)))))
                  (lambda (tab key)
                    (let lp ((prev tab) (rest (cdr tab)))
                      (match rest
                        ([]
                         (void))
                        ([entry . tail]
      ...
      (def ancestor-counts (make-tab))
      (def (get-ancestor-count c) (tab-ref ancestor-counts c 0))
      (def (increment-ancestor-count c) (tab-set! ancestor-counts c (1+ (get-ancestor-count c))))
      (def (decrement-ancestor-count c)
        (let ((count (1- (get-ancestor-count c))))
          (if (= 0 count)
            (tab-remove! ancestor-counts c)
            (tab-set! ancestor-counts c count))))
      ...
      ;; Update the suffix variable with each of the suffix specifications found.
                    ...
                    ([a . ar]
                     (if (suffix? a)
                       ...
                       (let (ms (merge-suffix a suffix))
                         (unless (and suffix (eq ms suffix))
                           ;; Newer longer suffix
                           (let loop2 ((tl al))
                             (match tl
                               ([] (void))
                               ([t . tr] (unless (and suffix (eq t suffix))
      ...
      (def suffix-tail-index (make-tab))
      (let loop ((i (length suffix-tail)) (t suffix-tail))
        (match t
          ([] (void))
          ([a . r]
           (tab-set! suffix-tail-index a i) (loop (1- i) r))))
                ...
                ((not p)
                 (let-values (((clrh2 h)
                               (append-reverse-until
                                (cut tab-ref suffix-tail-index <> #f) clrh [c])))
                   (if (null? clrh2)
                     h
                     (err2
          ...
          (match ts
            ([[c . _] . rts]
             (if (zero? (get-ancestor-count c))
               ...
               (loop rts)))
            (else
             (err ;; ancestor-counts: (tab->list ancestor-counts)
              c3-select-next: 'fail)))))
...
;; Algorithm to compute the slots of class (as a list, and as a reverse index table) given
;; - precedence-list: the precedence-list for the class, as computed by C4,
;;   excluding the newly defined class itself
;; - direct-slot-list: a list of symbols naming the slots defined in the class itself
;; - mixin-slot-names: a function that given a mixin (ancestor, element of the precedence-list)
;;     returns a list of symbols naming the slots defined in the mixin.
;; - result: a continuation function that takes as parameters the results and processes them;
;;     the results are a list of slots in order, starting with a special slot 'class
;;     a symbolic-table mapping symbols and their same-named keywords to the slot index
;;     in the same order as the list. Note that the special slot is not in this table;
;;     but a regular slot named 'class might. A slot name may only appear once in the tail
;;     of the list.
