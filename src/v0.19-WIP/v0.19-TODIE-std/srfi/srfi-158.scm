...
(define (list->bytevector list)
  (let ((vec (make-u8vector (length list) 0)))
    (let loop ((i 0) (list list))
      (if (null? list)
        vec
        (begin
          (u8vector-set! vec i (car list))
          (loop (+ i 1) (cdr list)))))))
...
#;(define bytevector->generator
  (case-lambda ((str) (bytevector->generator str 0 (u8vector-length str)))
               ((str start) (bytevector->generator str start (u8vector-length str)))
               ((str start end)
                (lambda () (if (>= start end)
                             (eof-object)
